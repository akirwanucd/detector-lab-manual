...
\subsection{TIFF files}
  ...
  Most typically we might see data delivered with the extensions \texttt{.tiff} or \texttt{.jp2}, however for our purposes we will be concerned with only the former. The so-called Tag Image File Format (TIFF) is particularly useful for storing raster graphics images, and like FITS files these can store data with a large bit-depth (typically 16 bit). Also similar to FITS files, TIFF files can actually contain seveeral images or \texttt{subfiles}, and support the assignment of multiple tags to store the file's metadata. 

  In the context of earth observations, a metadata standard that we will see applied to our Sentinel-2 data is called \href{https://www.earthdata.nasa.gov/about/esdis/esco/standards-practices/geotiff}{GeoTIFF}, which was developed as a way to process and store georeferenced raster images. This allows us to extract longitude and latitude values from our data, assuming geometric calibration was performed in the pipeline. For our purposes, we will be dealing with geometrically and radiometrically calibrated data, so we will not need to worry about it.
