...
\newcommand{\textgamma}{\ensuremath{\gamma}} % changed from \renewcommand 
...
\@mainmattertrue
\makeatother
\typeout{=== MAIN MATTER STARTED ===}

\chapter{Introduction to PHYC40870}\label{chap:intro}
...
\begin{figure}
    ...
    \caption{A \texttt{ValueError} raised by attempting to convert a \texttt{str}-type to an \texttt{int} type.\label{fig:python-exception01}}
\end{figure}
...
\begin{figure*}[h]
    ...
    \caption{A schematic demonstrating the ``branching`` of a repository; from GitHub.com.\label{fig:git-branch-flow}}
\end{figure*}
...
\begin{figure*}[h]
    ...
    \caption{A fairly typical workflow for a software project.\label{fig:complex-workflow}}
\end{figure*}
...
\begin{marginfigure}
    ...
    \caption{The \texttt{clone} options available on the browser interface of a GitHub repo.\label{fig:git-clone}}
\end{marginfigure}
...
%    \centering
    ...
    \caption{A visualization of the electromagnetic spectrum, highlighting the region of visible light to which the human eye is sensitive. Illustration by Philip Ronan, \href{https://commons.wikimedia.org/wiki/File:EM_spectrum.svg}{CC BY-SA 3.0}, obtained through Wikimedia Commons.\label{fig:em-spectrum}}
\end{figure*}
...
\begin{figure*}
    ...
    \begin{subfigure}{0.33\textwidth}
        ...
        \caption{}
    \end{subfigure}
    \caption{{\small Panel (a): Schematic of a section of a CCD pixel with four electrodes. The channel stop prevents charge leakage from one pixel column to another. Charges are stored in a buried channel separated from the surface by an insulator. Panel (b): Charges are moved across a pixel through clocking. Images courtesy of Teledyne Imaging.}\label{fig:ccd-array}}
\end{figure*}
...
\begin{figure}
   ...
   \caption{A cartoon demonstrating the charge-to-voltage conversion process for CCDs and CMOS detectors. Image courtesy of \href{https://meroli.web.cern.ch/meroli/lecture_cmos_vs_ccd_pixel_sensor.html}{Stefan Meroli/CERN}.\label{fig:ccd-cmos}}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{Transmission throughput curves for four HST filters, which are named based on the central wavelength and bandpass width (e.g. \textbf{W}ide or \textbf{N}arrow. For example, F475W signifies it is a filter with a central wavelength of $475$ nm and wide bandpass.\label{fig:hst-filters}}
\end{figure}

\begin{figure*}[h]
  \centering
  \includegraphics[width=0.9\linewidth]{graphics/ccds/S2-2022_Auto16.jpeg}
  \caption{Spatial resolution versus wavelength for the Multi-Spectral Imager (MSI) on-board Sentinel-2. The 13 spectral bands span the visible through the IR. Image courtesty of the \href{https://www.eoportal.org/satellite-missions/copernicus-sentinel-2}{eoPortal}.\label{fig:sen2-filters}}
\end{figure*}
...
\begin{figure}
    ...
    \caption{An image of the famous Pillars of Creation as seen by the WFPC2. The staircase pattern is seen clearly, and the regions of the different chips are overlaid on the image.\label{fig:wfpc2-chips}}
\end{figure}
...
\begin{marginfigure}
    ...
    \caption{The building blocks of a FITS file. Image courtesy of the Space Telescope Science Institute.\label{fig:fits-format}}
\end{marginfigure}
...
\begin{marginfigure}
    ...
    \caption{An example of an annular aperture. Small circular apertures (shown in white) are focused on the source, while smaller annuli (shown in red) encompass the local background. Image from the \href{https://photutils.readthedocs.io/en/stable/user_guide/aperture.html}{photutils documentation}.\label{fig:apertures}}
\end{marginfigure}
...
\begin{figure}
    ...
    \caption{The Sentinel-2 satellite\label{fig:sentinel}}
\end{figure}
...
\begin{figure*}
    ...
    \caption{Left: the decay scheme of $^{241}$Am to $^{237}$Np showing the 60 keV transition. Right: The decay scheme of $^{60}$Co to $^{60}$Ni, showing the prompt cascade of the 1173 and 1332 keV transitions. Images are modified from Brookhaven National Laboratory.\label{fig:decay-scheme}}
\end{figure*}
...
\begin{figure}[h!]
    ...
    \caption{An energy spectrum showing the 662 keV photo-peak of $^{137}$Cs as detected by a NaI:Tl detector. Figure originally from Oregon State University.\label{fig:cs137-spectra}}
\end{figure}
...
\chapter{AMSATs\label{chap:amsats}}
...
\begin{figure}
    ...
    \caption{A fully-assembled AMSAT, displaying the housing, a side-view of the PCB stack, and one of the side-mounted solar panels. A small RBF (``remove before flight'') pin can also be seen, which, when removed, triggers the RF mode and begins broadcasting telemetry packets over radio.\label{fig:amsat-assembled}}
\end{figure}
...
\begin{marginfigure}
    ...
    \caption{The Linux network dialogue showing the different connections and options available.\label{fig:linux-network}}
\end{marginfigure}
...
\begin{marginfigure}
    ...
    \caption{The Wired Connection settings window. The IPv4 method is Link-Local Only, and IPv6 is disabled.\label{fig:ipv4-config}}
\end{marginfigure}
% \begin{lstlisting}[backgroundcolor = \color{lightgray},caption=Example code,label=fig:header]
% Here is some example code of how to set up conda
% \end{lstlisting}
...
\begin{figure}
    ...
    \caption{A screenshot of the \texttt{raspi-config} interface. From here you can adjust several features of the Raspberry Pi.\label{fig:raspiconfig}}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{The \texttt{nano} text editor interface.\label{fig:nanoeditor}}
\end{figure}
...
\begin{marginfigure}
    ...
    \caption{A schematic of standard communication streams. you can read more about \href{https://www.linfo.org/standard_input.html}{standard input} and \href{https://www.linfo.org/standard_output.html}{standard output} if you are interested. Figure public domain courtesy of Wikipedia.\label{fig:iostreams}}
\end{marginfigure}
