...
\smallcaps{Authors: \\
                    ...
                    Sheila McBreen (sheila.mcbreen@ucd.ie, UCD School of Physics)\\
\smallskip
...
\smallcaps{version 4.0, 1 Sept 2026}\\
...
Aside from the major problem with using LLMs for writing, namely that it has no {\it understanding} of what it outputs\footnote{See \href{https://link.springer.com/article/10.1007/s10676-024-09775-5}{this article} and \href{https://link.springer.com/article/10.1007/s10676-025-09828-3}{this response} for an entertaining discussion on this point.}, there are other more subtle downsides:
...
% check out the links: https://sheffield-mps.github.io/PHY241/lectures/l06/
...
You can imagine that a CMOS detector works in much the same way: light strikes a pixel, the photon energy is converted to a proportionate number of electrons, and some conversion occurs that allows us to read the corresponding value. However, with the CCD the charge is moved accross pixels before being read out, whereas a CMOS detector allows each pixel to locally convert the charge to a digital voltage which is read out directly through a series of electronic switches. This provides a very rapid and power-efficient operation that also allows pixels to be read individually; while this historically came at the cost of high noise and low image resolution, recent advancements in the technology have seen it surpass the CCD in performance and quality.
...
For Earth observations, we use \textbf{geometric} calibration based on highly
detailed world maps to assign physical coordinates to pixels. This
is done alongside \textbf{radiometric} calibration, which is somewhat analogous
to photometric calibration above. With radiometric calibration, the effects of
the atmosphere are much more important, so it is critical to have maps of
aerosol content, cloud cover, and water vapor, and correlate these with solar
irradiance models and radiative transfer models in order to convert raw values
to physically meaningful surface reflectance.
...
For science images it is a bit more complicated, so much so that multiple robust algorithms have been developed (such as L.A.Cosmic; see \href{https://ui.adsabs.harvard.edu/abs/2001PASP..113.1420V/abstract}{van Dokkum, 2001}) to address it. We will not worry so much about that here. %see a few of these in the workshop. 
...
Like nearly all digital astronomical data, the data produced by these instruments are stored in standardized files which follow the Flexible Image Transport System (FITS) protocol. FITS files store their data in a monochromatic, multi-dimensional plain-text form as "header data units" or HDUs and combine them into an HDU list. Each HDU is stored as an extension and would contain the physical information (counts, fluxes, etc.; the numerical data) as well as a header that contains all the metadata about the physical information. A visualization of this can be seen in Figure~\ref{fig:fits-format}.
...
FITS files support a large bit depth, which means the tonal resolution (that is, the ability to distinguish between similar brightness levels) delivered can be very fine. Typical phone cameras may store 8-bit data, so every pixel is capable of storing 256 different values representing the brightness; the data produced by astronomical cameras are 16-bita resolution so each pixel can actually store 65,536 unique values. This kind of tonal resolution allows astronomerys to capture both incredibly bright and incredibly faint details of an object in a single image. 
...
You will be working with observations of the loose globular cluster NGC 1261 as observed with the WFPC2. This object was surveyed as part of a large legacy survey project aimed at better understanding stellar populations, which is detailed in \href{https://iopscience.iop.org/article/10.1088/0004-6256/149/3/91}{Piotto et al (2015)}. These data are stored in the FITS image format, which you can load in Python using the \verb|astropy| module. You are tasked with writing a Python script to accomplish the following:
%from the F336W and F555W filters, you will complete the following tasks:

\begin{enumerate}
  \item 
    Calibrate the three raw images provided by your instructor using the associated calibration files;
  \item 
    Stack the calibrated images to remove cosmic rays;
  \item 
    Compare your calibrated files against the science images in the F336W filter
\end{enumerate}

While it is not part of the submittable assessment, you should also stack the science images to remove cosmic rays before comparing them to your own calibrated files. How you compare the files is up to you, but your analysis should be \textbf{quantitative} and ideally make good use of figures.
\end{tcolorbox} 
...
The Sentinel-2B satellite (Fig. \ref{fig:sentinel}) was launched in 2017, and was later joined by the identical Sentinel-2C in 2024. Each of the Sentinel-2 satellites caries a single instrument, the ``Multi-Spectral Imager''. You can read the detailed of how the MSI works \href{https://sentiwiki.copernicus.eu/web/s2-mission#S2-Mission-MSI-Instrument}{here}, but in brief it uses a ``push-broom'' approach to scan a 290~km-wide swathe of the Earth's surface. This push-broom method is similar to how a photocopier or scanner works: as the satellite moves through its orbit, it scans a region of the earth beneath it and updates its detector register as it goes. This is seen in Figure~\ref{fig:pushbroom}.
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{graphics/ccds/push-broom-scanner.png}
  \caption{A schematic of a push-broom system on a satellite. The sensor moves over a swath or patch of land, depicted by the horizontal black bars, as it moves along its flight path. The detector register is updated for each patch passed over. Image adapted from \href{https://doi.org/10.3390/rs17030433}{Du, et al (2025)}\label{fig:pushbroom}.}
\end{figure}
...
\href{https://gisgeography.com/sentinel-2-bands-combinations}{link}. For ease of reference we have adapted the (very helpful) table from the linked site below.

\begin{table*}[htbp]
  \centering
  \begin{tabular*}{0.9\textwidth}{@{\extracolsep{\fill}}c c c c}
%  \begin{tabular*}{{\textwidth}{@{\extracolsep{\fill}}cccc}
    \toprule
    \textbf{Band} & \textbf{Resolution} & \textbf{Central Wavelength} & \textbf{Description} \\
    \midrule
    ...
    \bottomrule
  \end{tabular*}
  \caption{Sentinel-2 Spectral Bands Specification\label{tab:sentinel-bands}}
\end{table*}
...
\subsection{TIFF files}

  Most typically we might see data delivered with the extensions \texttt{.tiff} or \texttt{.jp2}, however for our purposes we will be concerned with only the former. The so-called Tag Image File Format (TIFF) is particularly useful for storing raster graphics images, and like FITS files these can store data with a large bit-depth (typically up to 16 bit) so we can obtain images with high tonal resolution. Also similar to FITS files, TIFF files can actually contain several images or \texttt{subfiles}, and support the assignment of multiple tags to store the file's metadata. 

  In the context of earth observations, a metadata standard that we will see applied to our Sentinel-2 data is called \href{https://www.earthdata.nasa.gov/about/esdis/esco/standards-practices/geotiff}{GeoTIFF}, which was developed as a way to process and store georeferenced raster images. This allows us to extract longitude and latitude values from our data, assuming geometric calibration was performed in the pipeline. In our workshop we will be dealing with geometrically and radiometrically calibrated data, so we will not need to worry about the calibration.
...
\begin{enumerate}
  \item Read in the images you have been provided with into numpy arrays. The \texttt{PIL} or \texttt{rasterio} packages can help with this.
    ...
    \item You have also been provided with the images that are used to make NDVI, NDWI etc (these are the files that have the suffix \texttt{.raw}. Use these to make your own NDWI and NDVI. How do these compare to the ones you have been provided with?
\end{enumerate}
